    ...
    for set in 0...NUMSETS
      ...
      if @sets[set].shadow
        if @sets[set].shadow.heart_gauge > 0
          ...
          if isPurifiable?(set)
            pbMessage(_INTL("{1} Anda di Ruang Pemurnian siap untuk dimurnikan!",@sets[set].shadow.name))
          end
        end
      end
    end
...
#    end
  ...
  def pbPlace(pkmn,position)
    ...
    if pkmn.egg?
      @scene.pbDisplay(_INTL("Tidak bisa menaruh telur di sana."))
      return false
    end
    if (position==0)
      if pkmn.shadowPokemon?
        ...
        @scene.pbDisplay(_INTL("Hanya Shadow Pokémon yang bisa pergi ke sana."))
        return false
      end
    elsif (position>=1)
      if pkmn.shadowPokemon?
  ...
  def pbOnPlace(pkmn)
    ...
    if @chamber.setCount(set)==0 && @chamber.isPurifiableIgnoreRegular?(set)
      ...
      @scene.pbDisplay(
         _INTL("{1} ini siap membuka hatinya. Namun, harus ada setidaknya satu Pokémon reguler di set untuk melakukan upacara pemurnian.",pkmn.name))
    end
  end
  ...
  def pbOpenSetDetail
    ...
    loop do
      ...
      if cmd[0]==0
        ...
        if curpkmn || heldpkmn
          commands=[_INTL("BERGERAK"),_INTL("RINGKASAN"),
             _INTL("MENARIK")]
          if curpkmn && heldpkmn
            commands[0]=_INTL("MENUKARKAN")
          elsif heldpkmn
            commands[0]=_INTL("TEMPAT")
          end
          cmdReplace=-1
          cmdRotate=-1
          if !heldpkmn && curpkmn && cmd[1]==0 &&
             @chamber[@chamber.currentSet].length>0
            commands[cmdRotate=commands.length]=_INTL("MEMUTAR")
          end
          if !heldpkmn && curpkmn
            commands[cmdReplace=commands.length]=_INTL("MENGGANTI")
          end
          commands.push(_INTL("MEMBATALKAN"))
          choice=@scene.pbShowCommands(
             _INTL("Apa yang harus saya lakukan dengan {1} ini?",
            ...
            if pos
              ...
              if newpkmn
                if (newpkmn.shadowPokemon?)!=(curpkmn.shadowPokemon?)
                  @scene.pbDisplay(_INTL("Pokemon itu tidak bisa ditempatkan di sana."))
                else
    ...
    if pbCheckPurify()
      @scene.pbDisplay(_INTL("Ada Pokémon yang siap membuka hatinya!"))
  ...
  def pbDoPurify
    ...
    for i in 0...purifiables.length
      ...
      if (i+1)!=purifiables.length
        @scene.pbDisplay(_INTL("Ada lagi Pokémon yang siap membuka hatinya!"))
    ...
    loop do
      ...
      if set<0
        ...
        cmd=@scene.pbShowCommands(
           _INTL("Apa yang ingin kamu lakukan?"),[
           _INTL("Sunting"),_INTL("MENGALIHKAN"),_INTL("MEMBATALKAN")])
        if cmd==0 # edit
          if !pbOpenSetDetail()
            break
          end
...
class PurifyChamberSetView < SpriteWrapper
  ...
  def refresh
    ...
    if pkmn
      ...
      textpos.push([_INTL("MENGALIR"),2+@info.bitmap.width/2,18,0,
         Color.new(248,248,248),Color.new(128,128,128)])
      # draw heart gauge
...
class PurifyChamberPC
  ...
  def name
    return _INTL("Ruang Pembersihan")
  end

  def access
    pbMessage(_INTL("\\se[PC access]Mengakses Ruang Pemurnian."))
    pbPurifyChamber()
  end
end
