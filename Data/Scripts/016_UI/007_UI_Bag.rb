...
class Window_PokemonBag < Window_DrawableCommand
  ...
  def drawItem(index,_count,rect)
    ...
    if index==self.itemCount-1
      textpos.push([_INTL("TUTUP TAS"),rect.x,rect.y-2,false,self.baseColor,self.shadowColor])
    else
...
class PokemonBag_Scene
  ...
  def pbStartScene(bag,choosing=false,filterproc=nil,resetpocket=true)
    ...
    @sliderbitmap = AnimatedBitmap.new(_INTL("Grafik/Gambar/Tas/icon_slider"))
    @pocketbitmap = AnimatedBitmap.new(_INTL("Grafik/Gambar/Tas/icon_pocket"))
    @sprites = {}
  ...
  def pbRefreshIndexChanged
    ...
    @sprites["itemtext"].text =
       (itemlist.item) ? GameData::Item.get(itemlist.item).description : _INTL("Tutup tas.")
  end
...
class PokemonBagScreen
  ...
  def pbStartScreen
    ...
    loop do
      ...
      commands[cmdRead = commands.length]       = _INTL("Membaca") if itm.is_mail?
      if ItemHandlers.hasOutHandler(item) || (itm.is_machine? && $Trainer.party.length>0)
        if ItemHandlers.hasUseText(item)
          ...
          commands[cmdUse = commands.length]    = _INTL("Menggunakan")
        end
      end
      commands[cmdGive = commands.length]       = _INTL("Memberi") if $Trainer.pokemon_party.length > 0 && itm.can_hold?
      commands[cmdToss = commands.length]       = _INTL("Melemparkan") if !itm.is_important? || $DEBUG
      if @bag.pbIsRegistered?(item)
        commands[cmdRegister = commands.length] = _INTL("Batalkan pilihan")
      elsif pbCanRegisterItem?(item)
        commands[cmdRegister = commands.length] = _INTL("Daftar")
      end
      commands[cmdDebug = commands.length]      = _INTL("Men-debug") if $DEBUG
      commands[commands.length]                 = _INTL("Batal")
      # Show commands generated above
      itemname = itm.name
      command = @scene.pbShowCommands(_INTL("{1} dipilih.",itemname),commands)
      if cmdRead>=0 && command==cmdRead   # Read mail
        ...
        if $Trainer.pokemon_count == 0
          @scene.pbDisplay(_INTL("Tidak ada Pokemon."))
        elsif itm.is_important?
          @scene.pbDisplay(_INTL("{1} tidak dapat diadakan.",itemname))
        else
          pbFadeOutIn {
        ...
        if qty>1
          helptext = _INTL("Buang berapa banyak {1}?",itm.name_plural)
          qty = @scene.pbChooseNumber(helptext,qty)
        end
        if qty>0
          itemname = itm.name_plural if qty>1
          if pbConfirm(_INTL("Bolehkah membuang {1} {2}?",qty,itemname))
            pbDisplay(_INTL("Membuang {1} {2}.",qty,itemname))
        ...
        command = 0
        loop do
          command = @scene.pbShowCommands(_INTL("Lakukan apa dengan {1}?",itemname),[
            _INTL("Ubah kuantitas"),
            _INTL("Buat Hadiah Misteri"),
            _INTL("Batal")
            ],command)
          case command
          ### Cancel ###
          ...
          when 0
            ...
            newqty = pbMessageChooseNumber(
               _INTL("Pilih kuantitas baru sebesar {1} (maks. #{Settings::BAG_MAX_PER_SLOT}).",itemplural),params) { @scene.pbUpdate }
            if newqty>qty
  ...
  def pbWithdrawItemScreen
    ...
    loop do
      ...
      if qty>1 && !itm.is_important?
        qty = @scene.pbChooseNumber(_INTL("Berapa banyak yang ingin Anda tarik?"),qty)
      end
      next if qty<=0
      if @bag.pbCanStore?(item,qty)
        ...
        pbDisplay(_INTL("Menarik {1} {2}.",dispqty,itemname))
      else
        pbDisplay(_INTL("Tidak ada lagi ruang di dalam Tas."))
      end
    end
    @scene.pbEndScene
  end
  ...
  def pbDepositItemScreen
    ...
    loop do
      ...
      qty = @bag.pbQuantity(item)
      if qty>1 && !itm.is_important?
        qty = @scene.pbChooseNumber(_INTL("Berapa banyak yang ingin Anda setorkan?"),qty)
      end
      if qty>0
        if !storage.pbCanStore?(item,qty)
          pbDisplay(_INTL("Tidak ada ruang untuk menyimpan barang."))
        else
          ...
          dispqty  = (itm.is_important?) ? 1 : qty
          itemname = (dispqty>1) ? itm.name_plural : itm.name
          pbDisplay(_INTL("Disetor {1} {2}.",dispqty,itemname))
        end
      end
    end
    @scene.pbEndScene
  ...
  def pbTossItemScreen
    ...
    loop do
      ...
      itm = GameData::Item.get(item)
      if itm.is_important?
        @scene.pbDisplay(_INTL("Itu terlalu penting untuk dibuang!"))
        next
      end
      qty = storage.pbQuantity(item)
      itemname       = itm.name
      ...
      if qty>1
        qty=@scene.pbChooseNumber(_INTL("Buang berapa banyak {1}?",itemnameplural),qty)
      end
      if qty>0
