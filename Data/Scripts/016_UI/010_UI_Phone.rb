...
class PokemonPhoneScene
  def start
    ...
    if @trainers.length==0
      pbMessage(_INTL("Tidak ada nomor telepon yang disimpan."))
      return
    end
    @sprites = {}
    ...
    @sprites["header"] = Window_UnformattedTextPokemon.newWithSize(_INTL("Telepon"),
    ...
    infotext = _INTL("Terdaftar<br>")
    infotext += _INTL("<r>{1}<br>",@sprites["list"].commands.length)
    infotext += _INTL("Menunggu pertandingan ulang<r>{1}",rematchcount)
