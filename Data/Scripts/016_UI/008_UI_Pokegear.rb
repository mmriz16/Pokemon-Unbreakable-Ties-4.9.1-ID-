...
class PokemonPokegearScreen
  ...
  def pbStartScreen
    ...
    commands[cmdMap = commands.length]     = ["map",_INTL("Peta")]
    if $PokemonGlobal.phoneNumbers && $PokemonGlobal.phoneNumbers.length>0
      commands[cmdPhone = commands.length] = ["phone",_INTL("Telepon")]
    end
    commands[cmdJukebox = commands.length] = ["jukebox",_INTL("kotak juke")]
    if defined?(Settings::ZUD_COMPAT)
      commands[cmdRaidData = commands.length] = ["database",_INTL("Basis Data Serangan")]
    end
