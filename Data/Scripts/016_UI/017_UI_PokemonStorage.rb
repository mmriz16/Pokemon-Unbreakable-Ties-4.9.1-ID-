...
class PokemonBoxPartySprite < SpriteWrapper
  ...
  def refresh
    ...
    pbDrawTextPositions(self.bitmap,[
       [_INTL("Kembali"), 86, 240, 2, Color.new(248, 248, 248), Color.new(80, 80, 80), 1]
    ])
...
class PokemonStorageScene
  ...
  def pbMark(selected,heldpoke)
    ...
    loop do
      ...
      if redraw
        ...
        textpos = [
           [_INTL("OKE"),402,208,2,base,shadow,1],
           [_INTL("Batal"),402,272,2,base,shadow,1]
        ]
  ...
  def pbUpdateOverlay(selection,party=nil)
    ...
    pbDrawTextPositions(overlay,[
       [_INTL("Pesta: {1}",(@storage.party.length rescue 0)),270,326,2,buttonbase,buttonshadow,1],
       [_INTL("KELUAR"),446,326,2,buttonbase,buttonshadow,1],
    ])
    pokemon = nil
    if @screen.pbHeldPokemon
    ...
    if !pokemon.egg?
      imagepos = []
      if pokemon.male?
        textstrings.push([_INTL("â"),148,2,false,Color.new(24,112,216),Color.new(136,168,208)])
      elsif pokemon.female?
        textstrings.push([_INTL("â"),148,2,false,Color.new(248,56,32),Color.new(224,152,144)])
      end
      ...
      if pokemon.ability
        ...
        textstrings.push([_INTL("Tidak ada kemampuan"),86,300,2,nonbase,nonshadow])
      end
      if pokemon.item
        textstrings.push([pokemon.item.name,86,336,2,base,shadow])
      else
        textstrings.push([_INTL("Tidak ada barang"),86,336,2,nonbase,nonshadow])
      end
      if pokemon.shiny?
...
class PokemonStorageScreen
  ...
  def pbStartScreen(command)
    ...
    if command==0   # Organise
      ...
      loop do
        ...
        if selected==nil
          if pbHeldPokemon
            pbDisplay(_INTL("Anda sedang memegang Pokemon!"))
            next
          end
          next if pbConfirm(_INTL("Lanjutkan operasi Box?"))
          break
          ...
          if pbConfirm(_INTL("Keluar dari Kotak?"))
            pbSEPlay("PC close")
            break
          end
          next
          ...
          if @scene.quickswap
            ...
            cmdCancel   = -1
            if heldpoke
              helptext = _INTL("{1} dipilih.",heldpoke.name)
              commands[cmdMove=commands.length]   = (pokemon) ? _INTL("Menggeser") : _INTL("Tempat")
            elsif pokemon
              helptext = _INTL("{1} dipilih.",pokemon.name)
              commands[cmdMove=commands.length]   = _INTL("Bergerak")
            end
            commands[cmdSummary=commands.length]  = _INTL("Ringkasan")
            commands[cmdWithdraw=commands.length] = (selected[0]==-1) ? _INTL("Toko") : _INTL("Menarik")
            commands[cmdItem=commands.length]     = _INTL("Barang")
            commands[cmdMark=commands.length]     = _INTL("Tanda")
            commands[cmdRelease=commands.length]  = _INTL("Melepaskan")
            commands[cmdDebug=commands.length]    = _INTL("Men-debug") if $DEBUG
            commands[cmdCancel=commands.length]   = _INTL("Batal")
      ...
      loop do
        ...
        if selected==nil
          ...
          case selected[0]
          when -2   # Party Pokémon
            pbDisplay(_INTL("Yang mana yang akan kamu ambil?"))
            next
          when -3   # Close box
            if pbConfirm(_INTL("Keluar dari Kotak?"))
              pbSEPlay("PC close")
          ...
          pokemon = @storage[selected[0],selected[1]]
          next if !pokemon
          command = pbShowCommands(_INTL("{1} dipilih.",pokemon.name),[
             _INTL("Menarik"),
             _INTL("Ringkasan"),
             _INTL("Tanda"),
             _INTL("Melepaskan"),
  ...
  def pbConfirm(str)
    return pbShowCommands(str,[_INTL("Ya"),_INTL("Tidak")])==0
  end
  ...
  def pbWithdraw(selected,heldpoke)
    ...
    index = selected[1]
    if box==-1
      raise _INTL("Tidak bisa mundur dari partai...");
    end
    if @storage.party_full?
      pbDisplay(_INTL("Pestamu penuh!"))
      return false
    end
  ...
  def pbStore(selected,heldpoke)
    ...
    index = selected[1]
    if box!=-1
      raise _INTL("Tidak dapat menyetor dari kotak...")
    end
    if pbAbleCount<=1 && pbAble?(@storage[box,index]) && !heldpoke
      pbPlayBuzzerSE
      pbDisplay(_INTL("Itu Pokémon terakhirmu!"))
    elsif heldpoke && heldpoke.mail
      pbDisplay(_INTL("Harap hapus Mail."))
    elsif !heldpoke && @storage[box,index].mail
      pbDisplay(_INTL("Harap hapus Mail."))
    else
      loop do
        destbox = @scene.pbChooseBox(_INTL("Deposit di Kotak yang mana?"))
        if destbox>=0
          firstfree = @storage.pbFirstFreePos(destbox)
          if firstfree<0
            pbDisplay(_INTL("Kotaknya penuh."))
            next
          end
  ...
  def pbPlace(selected)
    ...
    if @storage[box,index]
      raise _INTL("Posisi {1},{2} tidak kosong...",box,index)
    end
  ...
  def pbSwap(selected)
    ...
    index = selected[1]
    if !@storage[box,index]
      raise _INTL("Posisi {1},{2} kosong...",box,index)
    end
  ...
  def pbRelease(selected,heldpoke)
    ...
    return if !pokemon
    if pokemon.egg?
      pbDisplay(_INTL("Anda tidak bisa melepaskan Telur."))
      return false
    elsif pokemon.mail
      pbDisplay(_INTL("Harap hapus email tersebut."))
      return false
    end
    ...
    command = pbShowCommands(_INTL("Lepaskan Pokémon ini?"),[_INTL("Tidak"),_INTL("Ya")])
    if command==1
      ...
      @scene.pbRefresh
      pbDisplay(_INTL("{1} telah dirilis.",pkmnname))
      pbDisplay(_INTL("Sampai jumpa, {1}!",pkmnname))
      @scene.pbRefresh
    end
    return
  end
  ...
  def pbItem(selected,heldpoke)
    ...
    if pokemon.item
      ...
      if pbConfirm(_INTL("Ambil ini {1}?",itemname))
        if !$PokemonBag.pbStoreItem(pokemon.item)
          pbDisplay(_INTL("Tidak dapat menyimpan {1}.",itemname))
        else
          pbDisplay(_INTL("Mengambil {1}.",itemname))
          pokemon.item = nil
          @scene.pbHardRefresh
        end
      end
      ...
      if item
        ...
        pbDisplay(_INTL("{1} sekarang sedang diadakan.",itemname))
        @scene.pbHardRefresh
      end
    end
  end
  ...
  def pbBoxCommands
    commands = [
       _INTL("Melompat"),
       _INTL("kertas dinding"),
       _INTL("Nama"),
       _INTL("Batal"),
    ]
    command = pbShowCommands(
       _INTL("Apa yang ingin kamu lakukan?"),commands)
    case command
    when 0
      destbox = @scene.pbChooseBox(_INTL("Lompat ke Kotak yang mana?"))
      if destbox>=0
    ...
    when 1
      ...
      wpaper = pbShowCommands(_INTL("Pilih kertas dindingnya."),papers[0],index)
      if wpaper>=0
        @scene.pbChangeBackground(papers[1][wpaper])
      end
    when 2
      @scene.pbBoxName(_INTL("Nama kotak?"),0,12)
    end
  end
  ...
  def pbChoosePokemon(_party=nil)
    ...
    loop do
      ...
      if selected==nil
        ...
        commands[2] = _INTL("Toko") if selected[0]==-1
        helptext = _INTL("{1} dipilih.",pokemon.name)
        command = pbShowCommands(helptext,commands)
        case command
        when 0   # Select
