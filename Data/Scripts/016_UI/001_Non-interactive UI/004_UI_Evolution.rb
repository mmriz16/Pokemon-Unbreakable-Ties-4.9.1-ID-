...
class PokemonEvolutionScene
  ...
  def pbEvolution(cancancel=true)
    ...
    pbMessageDisplay(@sprites["msgwindow"],
       _INTL("\\se[]Apa? {1} sedang berkembang!\\^",@pokemon.name)) { pbUpdate }
    ...
    if canceled
      pbMessageDisplay(@sprites["msgwindow"],
         _INTL("Hah? {1} berhenti berevolusi!",@pokemon.name)) { pbUpdate }
    else
      pbEvolutionSuccess
    end
  end
  ...
  def pbEvolutionSuccess
    ...
    pbMessageDisplay(@sprites["msgwindow"],
       _INTL("\\se[]Selamat! {1} Anda berevolusi menjadi {2}!\\wt[80]",
