...
class PokemonTrade_Scene
  ...
  def pbTrade
    ...
    pbMessageDisplay(@sprites["msgwindow"],
       _ISPRINTF("{1:s}\r\nNomor ID: {2:05d} Lembur: {3:s}\\wtnp[0]",
       @pokemon.name,@pokemon.owner.public_id,@pokemon.owner.name)) { pbUpdate }
    pbMessageWaitForInput(@sprites["msgwindow"],50,true) { pbUpdate }
    pbPlayDecisionSE
    pbScene1
    pbMessageDisplay(@sprites["msgwindow"],
       _INTL("Untuk {2} {1},\r\n{3} mengirimkan {4}.",@trader1,speciesname1,@trader2,speciesname2)) { pbUpdate }
    pbMessageDisplay(@sprites["msgwindow"],
       _INTL("{1} mengucapkan selamat tinggal pada {2}.",@trader2,speciesname2)) { pbUpdate }
    pbScene2
    pbMessageDisplay(@sprites["msgwindow"],
       _ISPRINTF("{1:s}\r\nID: {2:05d} Lembur: {3:s}",
       @pokemon2.name,@pokemon2.owner.public_id,@pokemon2.owner.name)) { pbUpdate }
    pbMessageDisplay(@sprites["msgwindow"],
       _INTL("Jaga baik-baik {1}.",speciesname2)) { pbUpdate }
  end
end
...
def pbStartTrade(pokemonIndex,newpoke,nickname,trainerName,trainerGender=0)
  ...
  if newpoke.is_a?(Pokemon)
    ...
    raise _INTL("Spesies {1} tidak ada.", newpoke) if !species_data
