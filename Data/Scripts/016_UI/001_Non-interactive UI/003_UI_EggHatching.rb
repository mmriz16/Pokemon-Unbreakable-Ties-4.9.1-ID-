...
class PokemonEggHatch_Scene
  def pbStartScene(pokemon)
    ...
    @sprites["pokemon"].setSpeciesBitmap(@pokemon.species, @pokemon.gender,
                                         ...
                                         false, false, false, true)   # Egg sprite
  ...
  def pbMain
    ...
    @pokemon.name = nil
    pbMessage(_INTL("\\se[]{1} menetas dari Telur!\\wt[80]", @pokemon.name)) { update }
    if pbConfirmMessage(
        _INTL("Apakah Anda ingin memberi nama panggilan {1} yang baru menetas?", @pokemon.name)) { update }
      nickname = pbEnterPokemonName(_INTL("Nama panggilan {1}?", @pokemon.name),
...
def pbHatch(pokemon)
  ...
  if !pbHatchAnimation(pokemon)
    ...
    pbMessage(_INTL("..."))
    pbMessage(_INTL("... ... ... ....."))
    pbMessage(_INTL("{1} menetas dari Telur!", speciesname))
    if pbConfirmMessage(_INTL("Apakah Anda ingin memberi nama panggilan {1} yang baru menetas?", speciesname))
      nickname = pbEnterPokemonName(_INTL("Nama panggilan {1}?", speciesname),
