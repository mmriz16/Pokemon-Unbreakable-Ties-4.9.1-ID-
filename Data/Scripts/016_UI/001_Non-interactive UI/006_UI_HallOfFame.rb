...
class HallOfFame_Scene
  ...
  def writePokemonData(pokemon,hallNumber=-1)
    ...
    textPositions=[
       ...
       [_INTL("Nomor ID.{1}",pokemon.egg? ? "?????" : idno),
  ...
  def writeWelcome
    overlay=@sprites["overlay"].bitmap
    overlay.clear
    pbDrawTextPositions(overlay,[[_INTL("Selamat datang di Hall of Fame!"),
...
class HallOfFamePC
  ...
  def access
    pbMessage(_INTL("\\se[PC access]Mengakses Hall of Fame."))
    pbHallOfFamePC
  end
end
