...
class MoveRelearner_Scene
  ...
  def pbDrawMoveList
    ...
    textpos=[
       [_INTL("Ajarkan gerakan yang mana?"),16,2,0,Color.new(88,88,80),Color.new(168,184,184)]
    ]
    imagepos=[]
    yPos=76
    for i in 0...VISIBLEMOVES
      ...
      if moveobject
        ...
        if moveData.total_pp>0
          textpos.push([_INTL("hal"),112,yPos+32,0,Color.new(64,64,64),Color.new(176,176,176)])
    ...
    textpos.push([_INTL("KATEGORI"),272,108,0,Color.new(248,248,248),Color.new(0,0,0)])
    textpos.push([_INTL("KEKUATAN"),272,140,0,Color.new(248,248,248),Color.new(0,0,0)])
    textpos.push([basedamage<=1 ? basedamage==1 ? "???" : "---" : sprintf("%d",basedamage),
          468,140,2,Color.new(64,64,64),Color.new(176,176,176)])
    textpos.push([_INTL("KETEPATAN"),272,172,0,Color.new(248,248,248),Color.new(0,0,0)])
...
class MoveRelearnerScreen
  ...
  def pbStartScreen(pkmn)
    ...
    loop do
      ...
      if move
        if @scene.pbConfirm(_INTL("Ajarkan {1}?", GameData::Move.get(move).name))
          if pbLearnMove(pkmn, move)
            @scene.pbEndScene
            return true
          end
        end
      elsif @scene.pbConfirm(_INTL("Berhenti mencoba mengajarkan langkah baru ke {1}?", pkmn.name))
        @scene.pbEndScene
        return false
      end
    end
