...
class PokemonPartyCancelSprite < PokemonPartyConfirmCancelSprite
  def initialize(viewport=nil)
    super(_INTL("MEMBATALKAN"),398,328,false,viewport)
  end
end
...
class PokemonPartyConfirmSprite < PokemonPartyConfirmCancelSprite
  def initialize(viewport=nil)
    super(_INTL("MENGONFIRMASI"),398,308,true,viewport)
  end
end
...
class PokemonPartyCancelSprite2 < PokemonPartyConfirmCancelSprite
  def initialize(viewport=nil)
    super(_INTL("MEMBATALKAN"),398,346,true,viewport)
  end
end
  ...
  def refresh
    ...
    if @refreshBitmap
      ...
      if !@pokemon.egg?
        ...
        # Draw gender symbol
        if @pokemon.male?
          textpos.push([_INTL("â"),224,10,0,Color.new(0,112,248),Color.new(120,184,232)])
        elsif @pokemon.female?
          textpos.push([_INTL("â"),224,10,0,Color.new(232,32,16),Color.new(248,168,184)])
        end
        # Draw shiny icon
        if @pokemon.shiny?
          pbDrawImagePositions(@overlaysprite.bitmap,[[
             "Graphics/Pictures/shiny2",80,48,0,0,16,16]])
        end
      end
      pbDrawTextPositions(@overlaysprite.bitmap,textpos)
      # Draw level text
...
class PokemonPartyScreen
  ...
  def pbPokemonGiveScreen(item)
    @scene.pbStartScene(@party,_INTL("Berikan ke Pokemon yang mana?"))
  ...
  def pbPokemonGiveMailScreen(mailIndex)
    ...
    if pkmnid>=0
      ...
      if pkmn.hasItem? || pkmn.mail
        ...
        pbDisplay(_INTL("Surat ditransfer dari Kotak Surat."))
  ...
  def pbRefreshAnnotations(ableProc)   # For after using an evolution stone
    ...
    for pkmn in @party
      ...
      annot.push((elig) ? _INTL("MAMPU") : _INTL("TIDAK MAMPU"))
    end
  ...
  def pbPokemonMultipleEntryScreenEx(ruleset)
    annot = []
    statuses = []
    ordinals = [_INTL("TIDAK MEMENUHI SYARAT"), _INTL("TIDAK DIMASUKKAN"), _INTL("DILARANG")]
    positions = [_INTL("PERTAMA"), _INTL("KEDUA"), _INTL("KETIGA"), _INTL("KEEMPAT"),
                 _INTL("KELIMA"), _INTL("KEENAM"), _INTL("KETUJUH"), _INTL("KEDELAPAN"),
                 _INTL("KESEMBILAN"), _INTL("KESEPULUH"), _INTL("KESEBELAS"), _INTL("KEDUABELAS")]
    for i in 0...Settings::MAX_PARTY_SIZE
    ...
    @scene.pbStartScene(@party,_INTL("Pilih Pokémon dan konfirmasi."),annot,true)
    loop do
      realorder = []
      for i in 0...@party.length
      ...
      @scene.pbSetHelpText(_INTL("Pilih Pokémon dan konfirmasi."))
      pkmnid = @scene.pbChoosePokemon
      addedEntry = false
      ...
      if (statuses[pkmnid] || 0) == 1
        commands[cmdEntry = commands.length]   = _INTL("Pintu masuk")
      elsif (statuses[pkmnid] || 0) > 2
        commands[cmdNoEntry = commands.length] = _INTL("Dilarang masuk")
      end
      pkmn = @party[pkmnid]
      commands[cmdSummary = commands.length]   = _INTL("Ringkasan")
      commands[commands.length]                = _INTL("Batal")
      command = @scene.pbShowCommands(_INTL("Lakukan apa dengan {1}?",pkmn.name),commands) if pkmn
      if cmdEntry>=0 && command==cmdEntry
        if realorder.length>=ruleset.number && ruleset.number>0
          pbDisplay(_INTL("Tidak lebih dari {1} Pokémon yang boleh masuk.",ruleset.number))
        else
        ...
        @scene.pbSummary(pkmnid) {
          @scene.pbSetHelpText((@party.length>1) ? _INTL("Pilih Pokemon.") : _INTL("Pilih Pokémon atau batalkan."))
        }
      end
    end
    @scene.pbEndScene
  ...
  def pbChooseAblePokemon(ableProc,allowIneligible=false)
    ...
    @scene.pbStartScene(@party,
       (@party.length>1) ? _INTL("Pilih Pokemon.") : _INTL("Pilih Pokémon atau batalkan."),annot)
    loop do
      @scene.pbSetHelpText(
         (@party.length>1) ? _INTL("Pilih Pokemon.") : _INTL("Pilih Pokémon atau batalkan."))
      pkmnid = @scene.pbChoosePokemon
      break if pkmnid<0
      if !eligibility[pkmnid] && !allowIneligible
        pbDisplay(_INTL("Pokémon ini tidak dapat dipilih."))
      else
        ret = pkmnid
        break
      end
  ...
  def pbPokemonScreen
    @scene.pbStartScene(@party,
       (@party.length>1) ? _INTL("Pilih Pokemon.") : _INTL("Pilih Pokémon atau batalkan."),nil)
    loop do
      @scene.pbSetHelpText((@party.length>1) ? _INTL("Pilih Pokemon.") : _INTL("Pilih Pokémon atau batalkan."))
      ...
      if pkmnid.is_a?(Array) && pkmnid[0]==1   # Switch
        @scene.pbSetHelpText(_INTL("Pindah ke mana?"))
        oldpkmnid = pkmnid[1]
      ...
      commands[cmdRelearn = commands.length]      = _INTL("Pelajari kembali")
      if !pkmn.egg?
      ...
      commands[cmdSwitch = commands.length]       = _INTL("Ganti") if @party.length>1
      if !pkmn.egg?
        if pkmn.mail
          commands[cmdMail = commands.length]     = _INTL("Surat")
        else
          commands[cmdItem = commands.length]     = _INTL("Barang")
        end
          commands[cmdRename = commands.length]     = _INTL("Ganti nama")
      end
      commands[commands.length]                   = _INTL("Batal")
      command = @scene.pbShowCommands(_INTL("Lakukan apa dengan {1}?",pkmn.name),commands)
      havecommand = false
      cmdMoves.each_with_index do |cmd, i|
        ...
        if [:MILKDRINK, :SOFTBOILED].include?(pkmn.moves[i].id)
          ...
          if pkmn.hp<=amt
            pbDisplay(_INTL("HPnya tidak cukup..."))
            break
          end
          @scene.pbSetHelpText(_INTL("Gunakan di Pokemon yang mana?"))
          oldpkmnid = pkmnid
          loop do
            ...
            if pkmnid==oldpkmnid
              pbDisplay(_INTL("{1} tidak dapat menggunakan {2} sendiri!",pkmn.name,movename))
            elsif newpkmn.egg?
              pbDisplay(_INTL("{1} tidak dapat digunakan pada Telur!",movename))
            elsif newpkmn.hp==0 || newpkmn.hp==newpkmn.totalhp
              pbDisplay(_INTL("{1} tidak dapat digunakan pada Pokémon tersebut.",movename))
            else
              pkmn.hp -= amt
              hpgain = pbItemRestoreHP(newpkmn,amt)
              @scene.pbDisplay(_INTL("HP {1} dipulihkan sebesar {2} poin.",newpkmn.name,hpgain))
              pbRefresh
            end
      ...
      if cmdSummary>=0 && command==cmdSummary
        ...
        command = @scene.pbShowCommands(_INTL("Lakukan apa dengan surat?"),
           [_INTL("Membaca"),_INTL("Mengambil"),_INTL("Batal")])
        case command
        when 0   # Read
          pbFadeOutIn {
        ...
        cmdMoveItem  = -1
        # Build the commands
        itemcommands[cmdUseItem=itemcommands.length]  = _INTL("Menggunakan")
        itemcommands[cmdGiveItem=itemcommands.length] = _INTL("Memberi")
        itemcommands[cmdTakeItem=itemcommands.length] = _INTL("Mengambil") if pkmn.hasItem?
        itemcommands[cmdMoveItem=itemcommands.length] = _INTL("Bergerak") if pkmn.hasItem? &&
                                                                         !GameData::Item.get(pkmn.item).is_mail?
        itemcommands[itemcommands.length]             = _INTL("Batal")
        command = @scene.pbShowCommands(_INTL("Lakukan apa dengan suatu item?"),itemcommands)
        if cmdUseItem>=0 && command==cmdUseItem   # Use
          ...
          item = pkmn.item
          itemname = item.name
          @scene.pbSetHelpText(_INTL("Pindahkan {1} ke mana?",itemname))
          oldpkmnid = pkmnid
          loop do
            @scene.pbPreSelect(oldpkmnid)
            pkmnid = @scene.pbChoosePokemon(true,pkmnid)
            ...
            if newpkmn.egg?
              pbDisplay(_INTL("Telur tidak bisa menampung barang."))
            elsif !newpkmn.hasItem?
              newpkmn.item = item
              pkmn.item = nil
              @scene.pbClearSwitching
              pbRefresh
              pbDisplay(_INTL("{1} diberi {2} untuk dipegang.",newpkmn.name,itemname))
              break
            elsif GameData::Item.get(newpkmn.item).is_mail?
              pbDisplay(_INTL("Surat {1} harus dihapus sebelum diberikan item.",newpkmn.name))
            else
              newitem = newpkmn.item
              newitemname = newitem.name
              if newitem == :LEFTOVERS
                pbDisplay(_INTL("{1} sudah mengadakan beberapa {2}.",newpkmn.name,newitemname))
              elsif newitemname.starts_with_vowel?
                pbDisplay(_INTL("{1} sudah mengadakan {2}.",newpkmn.name,newitemname))
              else
                pbDisplay(_INTL("{1} sudah mengadakan {2}.",newpkmn.name,newitemname))
              end
              if pbConfirm(_INTL("Apakah Anda ingin mengganti kedua item tersebut?"))
                newpkmn.item = item
                pkmn.item = newitem
                @scene.pbClearSwitching
                pbRefresh
                pbDisplay(_INTL("{1} diberi {2} untuk dipegang.",newpkmn.name,itemname))
                pbDisplay(_INTL("{1} diberi {2} untuk dipegang.",pkmn.name,newitemname))
                break
              end
            end
          end
...
def pbChoosePokemon(variableNumber,nameVarNumber,ableProc=nil,allowIneligible=false)
  ...
  pbFadeOutIn {
    ...
    if ableProc
      ...
      screen.pbStartScene(_INTL("Pilih Pokemon."),false)
