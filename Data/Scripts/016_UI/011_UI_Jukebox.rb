...
class PokemonJukebox_Scene
  ...
  def pbStartScene(commands)
    ...
    @sprites["header"] = Window_UnformattedTextPokemon.newWithSize(
       _INTL("kotak juke"),2,-18,128,64,@viewport)
...
class PokemonJukeboxScreen
  ...
  def pbStartScreen
    ...
    cmdOak     = -1
    cmdCustom  = -1
    commands[cmdMarch = commands.length]   = _INTL("Berbaris")
    commands[cmdLullaby = commands.length] = _INTL("Nyanyian pengantar tidur")
    commands[cmdOak = commands.length]     = _INTL("Ek")
    commands[cmdCustom = commands.length]  = _INTL("Kebiasaan")
    commands[commands.length]              = _INTL("KELUAR")
