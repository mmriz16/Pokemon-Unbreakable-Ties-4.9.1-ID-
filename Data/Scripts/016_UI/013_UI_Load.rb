...
class PokemonLoadPanel < SpriteWrapper
  ...
  EASYMODE_TEXTCOLOR     = Color.new(207,243,172)
  EASYMODE_SHADOWCOLOR   = Color.new(73,124,58)

  NORMALMODE_TEXTCOLOR      = Color.new(255,242,167)
  NORMALMODE_SHADOWCOLOR    = Color.new(203,181,37)

  RADICALMODE_TEXTCOLOR    = Color.new(213,198,251)
  RADICALMODE_SHADOWCOLOR  = Color.new(106,71,145)

  def initialize(index,title,gameSwitchesGOD,isContinue,trainer,framecount,mapid,viewport=nil)
    super(viewport)
    @index = index
    @title = title
    @gameSwitchesGOD = gameSwitchesGOD
    echo gameSwitchesGOD
  ...
  def refresh
    ...
    if @refreshBitmap
      ...
      if @isContinue
        ...
        if @gameSwitchesGOD[2093] == true
          textpos.push(["Easy Mode",56*2,18*2,0,EASYMODE_TEXTCOLOR,EASYMODE_SHADOWCOLOR])
        elsif @gameSwitchesGOD[2095] == true && @gameSwitchesGOD[2097] == false
          textpos.push(["Hard Mode",56*2,18*2,0,FEMALETEXTCOLOR,FEMALETEXTSHADOWCOLOR])
        elsif @gameSwitchesGOD[2096] == true
          textpos.push(["Radical Mode",56*2,18*2,0,RADICALMODE_TEXTCOLOR,RADICALMODE_SHADOWCOLOR])
        elsif @gameSwitchesGOD[2097] == true
          textpos.push(["Lucario Mode",56*2,18*2,0,MALETEXTCOLOR,MALETEXTSHADOWCOLOR])
        else
          textpos.push(["Normal Mode",56*2,18*2,0,NORMALMODE_TEXTCOLOR,NORMALMODE_SHADOWCOLOR])
        end
        
        textpos.push([@title,16*2,2*2,0,TEXTCOLOR,TEXTSHADOWCOLOR])
        textpos.push([_INTL("Lencana:"),16*2,53*2,0,TEXTCOLOR,TEXTSHADOWCOLOR])
        textpos.push([@trainer.badge_count.to_s,103*2,53*2,1,TEXTCOLOR,TEXTSHADOWCOLOR])
        textpos.push([_INTL("Pokedex:"),16*2,69*2,0,TEXTCOLOR,TEXTSHADOWCOLOR])
        textpos.push([@trainer.pokedex.seen_count.to_s,103*2,69*2,1,TEXTCOLOR,TEXTSHADOWCOLOR])
        textpos.push([_INTL("Waktu:"),16*2,85*2,0,TEXTCOLOR,TEXTSHADOWCOLOR])
        hour = @totalsec / 60 / 60
        min  = @totalsec / 60 % 60
        if hour>0
          textpos.push([_INTL("{1}jam {2}m",hour,min),103*2,85*2,1,TEXTCOLOR,TEXTSHADOWCOLOR])
        else
...
class PokemonLoad_Scene
  def pbStartScene(commands,gameSwitchesGOD, show_continue, trainer, frame_count, map_id)
    @commands = commands
    @sprites = {}
    ...
    for i in 0...commands.length
      @sprites["panel#{i}"] = PokemonLoadPanel.new(i,commands[i],gameSwitchesGOD, (show_continue) ? (i==0) : false,trainer,frame_count,map_id,@viewport)
...
class PokemonLoadScreen
  ...
  def pbStartDeleteScreen
    ...
    @scene.pbStartScene2
    if SaveData.exists?
      if pbConfirmMessageSerious(_INTL("Hapus semua data yang disimpan?"))
        pbMessage(_INTL("Setelah data dihapus, tidak ada cara untuk memulihkannya."))
        if pbConfirmMessageSerious(_INTL("Tetap hapus data yang disimpan?"))
          pbMessage(_INTL("Menghapus semua data. Jangan matikan daya.\\wtnp[0]"))
          self.delete_save_data
        end
      end
    else
      pbMessage(_INTL("Tidak ada file simpanan yang ditemukan."))
    end
    @scene.pbEndScene
    $scene = pbCallTitle
  end
