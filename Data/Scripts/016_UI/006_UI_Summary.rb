...
class PokemonSummary_Scene
  ...
  def pbConfirm(text)
    ...
    @sprites["messagebox"].text    = text
    @sprites["messagebox"].visible = true
    using(cmdwindow = Window_CommandPokemon.new([_INTL("Ya"),_INTL("Tidak")])) {
  ...
  def drawPage(page)
    ...
    pagename = [_INTL("INFORMASI"),
                _INTL("MEMO PELATIH"),
                _INTL("KETERAMPILAN"),
                _INTL("BERGERAK"),
                _INTL("PITA"),
                _INTL("INFO KEMAMPUAN")][page-1]
    textpos = [
       ...
       [_INTL("Barang"),66,312,0,base,shadow]
    ]
    ...
    if @pokemon.hasItem?
      ...
      textpos.push([_INTL("Tidak ada"),16,346,0,Color.new(192,200,208),Color.new(208,216,224)])
    end
    # Write the gender symbol
    if @pokemon.male?
      textpos.push([_INTL("â"),178,56,0,Color.new(24,112,216),Color.new(136,168,208)])
    elsif @pokemon.female?
      textpos.push([_INTL("â"),178,56,0,Color.new(248,56,32),Color.new(224,152,144)])
    end
    # Draw all text
  ...
  def drawPageOne
    ...
    # Write various bits of text
    textpos = [
       [_INTL("Nomor Dex."),238,74,0,base,shadow],
       [_INTL("Jenis"),238,106,0,base,shadow],
       [@pokemon.speciesName,435,106,2,Color.new(64,64,64),Color.new(176,176,176)],
       [_INTL("Jenis"),238,138,0,base,shadow],
       [_INTL("PL"),238,170,0,base,shadow],
       [_INTL("nomor identitas."),238,202,0,base,shadow],
    ]
    ...
    if @pokemon.owner.name.empty?
      textpos.push([_INTL("PERSEWAAN"),435,170,2,Color.new(64,64,64),Color.new(176,176,176)])
    ...
    if @pokemon.shadowPokemon?
      textpos.push([_INTL("Pengukur Jantung"),238,234,0,base,shadow])
      heartmessage = [_INTL("Pintu menuju hatinya terbuka! Buka kunci terakhir!"),
                      _INTL("Pintu menuju hatinya hampir terbuka sepenuhnya."),
                      _INTL("Pintu menuju hatinya hampir terbuka."),
                      _INTL("Pintu menuju hatinya terbuka lebih lebar."),
                      _INTL("Pintu menuju hatinya terbuka."),
                      _INTL("Pintu menuju hatinya tertutup rapat.")][@pokemon.heartStage]
      memo = sprintf("<c3=404040,B0B0B0>%s\n",heartmessage)
      drawFormattedTextEx(overlay,234,304,264,memo)
    else
      endexp = @pokemon.growth_rate.minimum_exp_for_level(@pokemon.level + 1)
      textpos.push([_INTL("Contoh. Poin"),238,234,0,base,shadow])
      textpos.push([@pokemon.exp.to_s_formatted,488,266,1,Color.new(64,64,64),Color.new(176,176,176)])
      textpos.push([_INTL("Ke Lv Berikutnya."),238,298,0,base,shadow])
  ...
  def drawPageOneEgg
    ...
    # Write various bits of text
    textpos = [
       [_INTL("MEMO PELATIH"),26,10,0,base,shadow],
       [@pokemon.name,46,56,0,base,shadow],
       [_INTL("Barang"),66,312,0,base,shadow]
    ]
    # Write the held item's name
    ...
    if mapname && mapname != ""
      memo += _INTL("<c3=404040,B0B0B0>Telur Pokémon misterius yang diterima dari <c3=F83820,E09890>{1}<c3=404040,B0B0B0>.",mapname)
    else
      memo += _INTL("<c3=404040,B0B0B0>Telur Pokémon yang misterius.",mapname)
    end
    memo += "\n" # Empty line
    # Write Egg Watch blurb
    memo += _INTL("<c3=404040,B0B0B0>\"Jam Tangan Telur\"")
    eggstate = _INTL("Sepertinya Telur ini akan memakan waktu lama untuk menetas.")
    eggstate = _INTL("Apa yang akan menetas dari ini? Tampaknya belum hampir menetas.") if @pokemon.steps_to_hatch < 10200
    eggstate = _INTL("Tampaknya bergerak sesekali. Mungkin hampir menetas.") if @pokemon.steps_to_hatch < 2550
    eggstate = _INTL("Suara terdengar datang dari dalam! Ini akan segera menetas!") if @pokemon.steps_to_hatch < 1275
    memo += sprintf("<c3=404040,B0B0B0>%s\n",eggstate)
    # Draw all text
  ...
  def drawPageTwo
    ...
    if showNature
      ...
      memo += _INTL("<c3=F83820,E09890>{1}<c3=404040,B0B0B0> alam.",natureName)
    end
    # Write date received
    if @pokemon.timeReceived
      date  = @pokemon.timeReceived.day
    ...
    mapname = pbGetMapNameFromId(@pokemon.obtain_map)
    mapname = @pokemon.obtain_text if @pokemon.obtain_text && !@pokemon.obtain_text.empty?
    mapname = _INTL("Tempat yang jauh") if nil_or_empty?(mapname)
    memo += sprintf("<c3=F83820,E09890>%s\n",mapname)
    # Write how Pokémon was obtained
    mettext = [_INTL("Bertemu di Lv. {1}.",@pokemon.obtain_level),
               _INTL("Telur diterima."),
               _INTL("Diperdagangkan di Lv. {1}.",@pokemon.obtain_level),
               "",
               _INTL("Mengalami pertemuan yang menentukan di Lv. {1}.",@pokemon.obtain_level)
    ...
    if @pokemon.obtain_method == 1
      ...
      mapname = _INTL("Tempat yang jauh") if nil_or_empty?(mapname)
      memo += sprintf("<c3=F83820,E09890>%s\n",mapname)
      memo += _INTL("<c3=404040,B0B0B0>Telur menetas.")
    else
      memo += "\n"   # Empty line
    end
    ...
    if showNature
      ...
      characteristics = {
        :HP              => [_INTL("Suka makan."),
                             _INTL("Butuh banyak tidur siang."),
                             _INTL("Banyak mengangguk."),
                             _INTL("Sering menyebarkan sesuatu."),
                             _INTL("Suka bersantai.")],
        :ATTACK          => [_INTL("Bangga dengan kekuatannya."),
                             _INTL("Suka ribut-ribut."),
                             _INTL("Sedikit cepat marah."),
                             _INTL("Suka berkelahi."),
                             _INTL("Cepat marah.")],
        :DEFENSE         => [_INTL("Tubuh kokoh."),
                             _INTL("Mampu menerima pukulan."),
                             _INTL("Sangat gigih."),
                             _INTL("Daya tahan yang baik."),
                             _INTL("Ketekunan yang bagus.")],
        :SPECIAL_ATTACK  => [_INTL("Sangat penasaran."),
                             _INTL("Nakal."),
                             _INTL("Benar-benar licik."),
                             _INTL("Sering tenggelam dalam pikiran."),
                             _INTL("Sangat rewel.")],
        :SPECIAL_DEFENSE => [_INTL("Berkemauan keras."),
                             _INTL("Agak sia-sia."),
                             _INTL("Sangat menantang."),
                             _INTL("Benci untuk kalah."),
                             _INTL("Agak keras kepala.")],
        :SPEED           => [_INTL("Suka berlari."),
                             _INTL("Waspada terhadap suara."),
                             _INTL("Ceroboh dan konyol."),
                             _INTL("Agak seperti badut."),
                             _INTL("Cepat melarikan diri.")]
      }
  ...
  def drawPageThree
    ...
    textpos = [
       ...
       [_INTL("Menyerang"),248,114,0,base,statshadows[:ATTACK]],
       [sprintf("%d",@pokemon.attack),456,114,1,Color.new(64,64,64),Color.new(176,176,176)],
       [_INTL("Pertahanan"),248,146,0,base,statshadows[:DEFENSE]],
       [sprintf("%d",@pokemon.defense),456,146,1,Color.new(64,64,64),Color.new(176,176,176)],
       [_INTL("Sp. Serangan"),248,178,0,base,statshadows[:SPECIAL_ATTACK]],
       ...
       [_INTL("Kecepatan"),248,242,0,base,statshadows[:SPEED]],
       [sprintf("%d",@pokemon.speed),456,242,1,Color.new(64,64,64),Color.new(176,176,176)],
       [_INTL("Kemampuan"),224,278,0,base,shadow]
    ]
  ...
  def drawPageFour
    ...
    for i in 0...Pokemon::MAX_MOVES
      ...
      if move
        ...
        if move.total_pp>0
          textpos.push([_INTL("hal"),342,yPos+32,0,moveBase,moveShadow])
          ppfraction = 0
  ...
  def drawPageFourSelecting(move_to_learn)
    ...
    # Write various bits of text
    textpos = [
       [_INTL("BERGERAK"),26,10,0,base,shadow],
       [_INTL("KATEGORI"),20,116,0,base,shadow],
       [_INTL("KEKUATAN"),20,148,0,base,shadow],
       [_INTL("KETEPATAN"),20,180,0,base,shadow]
    ]
    imagepos = []
    # Write move names, types and PP amounts for each known move
    yPos = 92
  ...
  def drawPageFive
    ...
    # Write various bits of text
    textpos = [
       [_INTL("Jumlah Pita:"),234,326,0,Color.new(64,64,64),Color.new(176,176,176)],
  ...
  def pbMarking(pokemon)
    ...
    loop do
      ...
      if redraw
        ...
        textpos = [
           [_INTL("Tandai {1}",pokemon.name),366,90,2,base,shadow],
           [_INTL("OKE"),366,242,2,base,shadow],
           [_INTL("Batal"),366,292,2,base,shadow]
        ]
  ...
  def pbOptions
    ...
    cmdMark     = -1
    if !@pokemon.egg?
      commands[cmdGiveItem = commands.length] = _INTL("Berikan barang")
      commands[cmdTakeItem = commands.length] = _INTL("Ambil barangnya") if @pokemon.hasItem?
      commands[cmdPokedex = commands.length]  = _INTL("Lihat PokÃ©dex") if $Trainer.has_pokedex
    end
    commands[cmdMark = commands.length]       = _INTL("Tanda")
    commands[commands.length]                 = _INTL("Batal")
...
class PokemonSummaryScreen
  ...
  def pbStartForgetScreen(party,partyindex,move_to_learn)
    ...
    loop do
      ...
      pbMessage(_INTL("Jurus HM kini tak bisa dilupakan.")) { @scene.pbUpdate }
    end
    @scene.pbEndScene
    return ret
  end
  ...
  def pbStartChooseMoveScreen(party,partyindex,message)
    ...
    loop do
      ...
      break if ret >= 0
      pbMessage(_INTL("Anda harus memilih langkah!")) { @scene.pbUpdate }
    end
    @scene.pbEndScene
    return ret
  end
