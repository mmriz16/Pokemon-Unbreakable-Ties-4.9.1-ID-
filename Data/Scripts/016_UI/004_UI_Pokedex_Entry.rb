...
class PokemonPokedexInfo_Scene
  ...
  def pbUpdateDummyPokemon
    ...
    if @sprites["formback"]
      @sprites["formback"].setSpeciesBitmap(@species,@gender,@form,false,false,false,true)
  ...
  def pbGetAvailableForms
    ...
    ret.each do |entry|
      if !entry[0] || entry[0].empty?   # Necessarily applies only to form 0
        case entry[1]
        when 0 then entry[0] = _INTL("Pria")
        when 1 then entry[0] = _INTL("Perempuan")
        else
          entry[0] = (multiple_forms) ? _INTL("Bentuk Dasar") : _INTL("Tanpa gender")
        end
      end
  ...
  def drawPageInfo
    @sprites["background"].setBitmap(_INTL("Grafik/Gambar/Pokedex/bg_info"))
    overlay = @sprites["overlay"].bitmap
    base   = Color.new(88, 88, 80)
    shadow = Color.new(168, 184, 184)
    imagepos = []
    if @brief
      imagepos.push([_INTL("Grafik/Gambar/Pokedex/overlay_info"), 0, 0])
    end
    species_data = GameData::Species.get_species_form(@species, @form)
    ...
    if !@show_number_battled
      textpos.push([_INTL("Tinggi"), 314, 152, 0, base, shadow])
      textpos.push([_INTL("Berat"), 314, 184, 0, base, shadow])
    else
      textpos.push([_INTL("Pertarungan Nomor:"), 314, 152, 0, base, shadow])
    ...
    if $Trainer.owned?(@species)
      ...
      if !@show_number_battled
        ...
        if System.user_language[3..4] == "US"   # If the user is in the United States
          ...
          textpos.push([_ISPRINTF("{1:4.1f} pon.", pounds / 10.0), 494, 184, 1, base, shadow])
        else
      ...
      if !@show_number_battled
        ...
        if System.user_language[3..4] == "US"   # If the user is in the United States
          ...
          textpos.push([_INTL("???.? pon."), 494, 184, 1, base, shadow])
        else
  ...
  def drawPageArea
    @sprites["background"].setBitmap(_INTL("Grafik/Gambar/Pokedex/bg_area"))
    overlay = @sprites["overlay"].bitmap
    base   = Color.new(88,88,80)
    shadow = Color.new(168,184,184)
    ...
    if points.length==0
      ...
      textpos.push([_INTL("Daerah tidak diketahui"),Graphics.width/2,Graphics.height/2 - 6,2,base,shadow])
    end
    textpos.push([pbGetMessage(MessageTypes::RegionNames,@region),414,38,2,base,shadow])
    textpos.push([_INTL("daerah {1}",GameData::Species.get(@species).name),
       Graphics.width/2,346,2,base,shadow])
    pbDrawTextPositions(overlay,textpos)
  end

  def drawPageForms
    @sprites["background"].setBitmap(_INTL("Grafik/Gambar/Pokedex/bg_forms"))
    overlay = @sprites["overlay"].bitmap
    base   = Color.new(88,88,80)
    shadow = Color.new(168,184,184)
