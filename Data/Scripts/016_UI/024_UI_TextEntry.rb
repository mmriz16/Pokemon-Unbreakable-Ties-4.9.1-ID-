...
class Window_CharacterEntry < Window_DrawableCommand
  ...
  def drawItem(index,_count,rect)
    ...
    if index==@charset.length # -1
      ...
      pbDrawShadowText(self.contents,rect.x,rect.y,rect.width,rect.height,_INTL("OKE"),
         self.baseColor,self.shadowColor)
    else
  ...
  def pbStartScene(helptext,minlength,maxlength,initialText,subject=0,pokemon=nil)
    ...
    if minlength==0
      @sprites["helpwindow"]=Window_UnformattedTextPokemon.newWithSize(
         _INTL("Masukkan teks menggunakan keyboard. Tekan\nEnter untuk mengonfirmasi, atau Esc untuk membatalkan."),
         32,Graphics.height-96,Graphics.width-64,96,@viewport
      )
    else
      @sprites["helpwindow"]=Window_UnformattedTextPokemon.newWithSize(
         _INTL("Masukkan teks menggunakan keyboard.\nTekan Enter untuk mengonfirmasi."),
         32,Graphics.height-96,Graphics.width-64,96,@viewport
      )
    end
    ...
    when 2   # Pokémon
      if pokemon
        ...
        textpos=[]
        if pokemon.male?
          textpos.push([_INTL("â"),0,-6,false,Color.new(0,128,248),Color.new(168,184,184)])
        elsif pokemon.female?
          textpos.push([_INTL("â"),0,-6,false,Color.new(248,24,24),Color.new(168,184,184)])
        end
...
class PokemonEntryScene2
  @@Characters = [
    [("ABCDEFGHIJ ,." + "KLMNOPQRST '-" + "UVWXYZ     ♂♀" + "             " + "0123456789   ").scan(/./), _INTL("ATAS")],
    [("abcdefghij ,." + "klmnopqrst '-" + "uvwxyz     ♂♀" + "             " + "0123456789   ").scan(/./), _INTL("lebih rendah")],
    [("ÀÁÂÄÃàáâäã Ææ" + "ÈÉÊË èéêë  Çç" + "ÌÍÎÏ ìíîï  Ññ" + "ÒÓÔÖÕòóôöõ Ýý" + "ÙÚÛÜ ùúûü    ").scan(/./), _INTL("aksen")],
    [(",.'\":;!?¡¿ ♂♀" + "~@#*&$µ¶§    " + "()[]{}<>«»   " + "+-×÷=±%¹²³¼½¾" + "^_/\\|        ").scan(/./), _INTL("lainnya")]
  ]
  ROWS    = 13
  COLUMNS = 5
  MODE1   = -6
