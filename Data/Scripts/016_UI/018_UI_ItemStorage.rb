...
class Window_PokemonItemStorage < Window_DrawableCommand
  ...
  def drawItem(index,_count,rect)
    ...
    textpos = []
    if index==@bag.length
      textpos.push([_INTL("MEMBATALKAN"),rect.x,rect.y-6,false,self.baseColor,self.shadowColor])
    else
      item     = @bag[index][0]
...
class ItemStorage_Scene
  ...
  def pbRefresh
    ...
    if itemwindow.item
      ...
      @sprites["itemtextwindow"].text = _INTL("Tutup penyimpanan.")
    end
    itemwindow.refresh
  end
