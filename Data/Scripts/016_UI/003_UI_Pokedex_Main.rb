...
class PokemonPokedex_Scene
  ...
  def pbStartScene
    ...
    @typebitmap         = AnimatedBitmap.new(_INTL("Grafik/Gambar/Pokedex/icon_types"))
    ...
    @searchsliderbitmap = AnimatedBitmap.new(_INTL("Grafik/Gambar/Pokedex/icon_searchslider"))
    @sprites = {}
...
=end
  ...
  def pbRefresh
    ...
    if @searchResults
      textpos.push([_INTL("Hasil pencarian"),112,302,2,base,shadow])
      textpos.push([@dexlist.length.to_s,112,334,2,base,shadow])
    else
      textpos.push([_INTL("Terlihat:"),42,302,0,base,shadow])
      textpos.push([$Trainer.pokedex.seen_count(pbGetPokedexRegion).to_s,182,302,1,base,shadow])
      textpos.push([_INTL("Dimiliki:"),42,334,0,base,shadow])
  ...
  def pbRefreshDexSearch(params,_index)
    ...
    # Write various bits of text
    textpos = [
       [_INTL("Modus Pencarian"),Graphics.width/2,-2,2,base,shadow],
       [_INTL("Memesan"),136,52,2,base,shadow],
       [_INTL("Nama"),58,110,2,base,shadow],
       [_INTL("Jenis"),58,162,2,base,shadow],
       [_INTL("Tinggi"),58,214,2,base,shadow],
       [_INTL("Berat"),58,266,2,base,shadow],
       [_INTL("Warna"),326,110,2,base,shadow],
       [_INTL("Membentuk"),454,162,2,base,shadow],
       [_INTL("Mengatur ulang"),80,338,2,base,shadow,1],
       [_INTL("Awal"),Graphics.width/2,338,2,base,shadow,1],
       [_INTL("Batal"),Graphics.width-80,338,2,base,shadow,1]
    ]
  ...
  def pbRefreshDexSearchParam(mode,cmds,sel,_index)
    ...
    textpos = [
       [_INTL("Modus Pencarian"),Graphics.width/2,-2,2,base,shadow],
       [_INTL("OKE"),80,338,2,base,shadow,1],
       [_INTL("Batal"),Graphics.width-80,338,2,base,shadow,1]
    ]
    title = [_INTL("Memesan"),_INTL("Nama"),_INTL("Jenis"),_INTL("Tinggi"),
             _INTL("Berat"),_INTL("Warna"),_INTL("Membentuk")][mode]
  ...
  def pbDexSearch
    ...
    @orderCommands = []
    @orderCommands[MODENUMERICAL] = _INTL("numerik")
    @orderCommands[MODEATOZ]      = _INTL("A sampai Z")
    @orderCommands[MODEHEAVIEST]  = _INTL("Terberat")
    @orderCommands[MODELIGHTEST]  = _INTL("Paling ringan")
    @orderCommands[MODETALLEST]   = _INTL("Tertinggi")
    @orderCommands[MODESMALLEST]  = _INTL("Terkecil")
    @nameCommands = [_INTL("A"),_INTL("B"),_INTL("C"),_INTL("D"),_INTL("E"),
                    _INTL("F"),_INTL("G"),_INTL("H"),_INTL("SAYA"),_INTL("J"),
                    _INTL("K"),_INTL("L"),_INTL("M"),_INTL("N"),_INTL("HAI"),
                    _INTL("P"),_INTL("Q"),_INTL("R"),_INTL("S"),_INTL("T"),
                    _INTL("kamu"),_INTL("V"),_INTL("W"),_INTL("X"),_INTL("Y"),
                    _INTL("Z")]
    @typeCommands = []
    ...
    loop do
      ...
      if Input.trigger?(Input::UP)
        ...
        when 8   # Start search (filter)
          ...
          if dexlist.length==0
            pbMessage(_INTL("Tidak ditemukan Pokémon yang cocok."))
          else
            @dexlist = dexlist
            @sprites["pokedex"].commands = @dexlist
