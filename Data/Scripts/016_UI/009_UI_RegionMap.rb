...
class PokemonRegionMap_Scene
  ...
  def pbStartScene(aseditor=false,mode=0)
    ...
    if !@map
      pbMessage(_INTL("Data peta tidak dapat ditemukan."))
      return false
    end
  ...
  def pbChangeMapLocation(x,y)
    ...
    currentname = pbMessageFreeText(_INTL("Tetapkan nama untuk titik ini."),currentname,false,250) { pbUpdate }
    if currentname
      if currentobj
  ...
  def pbMapScene(mode=0)
    ...
    loop do
      ...
      if Input.trigger?(Input::BACK)
        if @editor && @changed
          if pbConfirmMessage(_INTL("Simpan perubahan?")) { pbUpdate }
            pbSaveMapData
          end
          if pbConfirmMessage(_INTL("Keluar dari peta?")) { pbUpdate }
            break
          end
        else
          break
