...
def pbEmergencySave
  oldscene = $scene
  $scene = nil
  pbMessage(_INTL("Naskahnya memakan waktu terlalu lama. Game akan dimulai ulang."))
  return if !$Trainer
  if SaveData.exists?
  ...
  if Game.save
    pbMessage(_INTL("\\se[]Game telah disimpan.\\me[GUI save game] File penyimpanan sebelumnya telah dicadangkan.\\wtnp[30]"))
  else
    pbMessage(_INTL("\\se[]Penyimpanan gagal.\\wtnp[30]"))
  end
  $scene = oldscene
end
...
class PokemonSave_Scene
  def pbStartScreen
    ...
    loctext+=_INTL("Pemain<r><c3={1}>{2}</c3><br>",textColor,$Trainer.name)
    if hour>0
      loctext+=_INTL("Waktu<r><c3={1}>{2}jam {3}m</c3><br>",textColor,hour,min)
    else
      loctext+=_INTL("Waktu<r><c3={1}>{2}m</c3><br>",textColor,min)
    end
...
class PokemonSaveScreen
  ...
  def pbSaveScreen
    ...
    if pbConfirmMessage(_INTL('Would you like to save the game?'))
      ...
      pbSEPlay('GUI save choice')
      if Game.save
        pbMessage(_INTL("\\se[]{1} menyelamatkan permainan.\\me[GUI save game]\\wtnp[30]", $Trainer.name))
        ret = true
      else
        pbMessage(_INTL("\\se[]Penyimpanan gagal.\\wtnp[30]"))
        ret = false
